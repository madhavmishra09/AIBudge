...
import { signupUser, setAuthToken } from "./authService";
...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    const name = [formData.fname, formData.mname, formData.lname].filter(Boolean).join(" ");
  ...
  return (
    <div className="font-mont flex flex-col items-center justify-center min-h-screen px-4 bg-purple-50">
      <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 rounded-lg shadow-lg w-full max-w-sm sm:max-w-md md:max-w-lg">
        ...
        <input type="text" name="fname" id="fname" placeholder="Enter Your First Name" value={formData.fname} onChange={handleChange} required className="font-mont border border-gray-300 rounded-md py-2 px-4 mb-4 focus:outline-none focus:ring-2 focus:ring-fuchsia-500 transition-all duration-300 w-full" />
